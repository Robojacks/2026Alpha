...
public class PhotonVisionSubsystem extends SubsystemBase {
  ...
  private PhotonTrackedTarget lastBestTarget;

  private Transform3d leftCameraTransform =
      new Transform3d(
          new Translation3d(
              Units.inchesToMeters(-4), Units.inchesToMeters(0), Units.inchesToMeters(20.5)),
          new Rotation3d(0, 15, Units.degreesToRadians(0)));
  ...
  */
  ...
  public PhotonVisionSubsystem(DriveSubsystem driveSubsystem) {
  ...
  /**
   * @return Field-relative position of the best detected target, or null if none.
   */
  public Translation2d getTargetPosition() {
    if (lastBestTarget == null) {
      return null;
    }
    Optional<Pose3d> tagPose = fieldLayout.getTagPose(lastBestTarget.getFiducialId());
    return tagPose.map(p -> p.getTranslation().toTranslation2d()).orElse(null);
  }
